...
struct tcb
{
  ...
  uint32_t sleep;   // thread-nap time
  uint32_t priority;
  int hasRun;
};
typedef struct tcb tcbType;

// #define NUMPERIODICTHREADS 4 // maximum number of periodic threads
// struct ptcb
// {
//   void (*func)(void);
//   int32_t period;
...
#define MAILBOX_SIZE 8 // maximum size of mailbox

struct fifoitem {
  uint32_t data;
  struct fifoitem *next;
};
typedef struct fifoitem fifoItemType;
#define FIFO_SIZE 10
